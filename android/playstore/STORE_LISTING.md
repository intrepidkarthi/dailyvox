...
```
`29 characters.` "Private" earns its slot: it is the search term this audience
actually types, and it is the one claim the listing can prove rather than assert.

## Short description (80 characters max)

Play shows this above the fold, before anyone taps *Read more*. It matters more
here than the iOS subtitle does.
...
The iOS subtitle is "Private On-Device Audio Diary" — three adjectives and a
noun. Every competitor can write that sentence. **This one names a verifiable
fact and invites the reader to go and check it**, which is a thing only an app
with an empty permission list can say.
...
Most journal apps promise privacy in their marketing. This one can be checked in
about ten seconds, without trusting a word of this page:
...
its default it would copy your journal and your recordings to Google Drive —
...
There is no free-form chatbot here, and that is a decision rather than a missing
feature. A model small enough to run on every Android phone would guess, and a
Twin that guesses about your own life is worse than one that stays quiet.
...
Free forever. No paid tier exists. The app's source code is public, so anyone
can verify how it handles your words.
...
| Tags | Journal, Mental wellness, Productivity |
| Contains ads | **No** |
| In-app purchases | **No** |
